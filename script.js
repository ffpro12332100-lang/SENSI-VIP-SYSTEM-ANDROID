...
function iniciarCarga(alFinalizar) {
    ...
    const estadosTexto = [
        { min: 0, texto: '⚡ INICIALIZANDO ALGORITMO...' },
        { min: 30, texto: '⚙️ AJUSTANDO PARÁMETROS PRO...' },
        { min: 70, texto: '🎯 CALCULANDO SENSIBILIDAD...' },
        { min: 95, texto: '✅ OPTIMIZACIÓN COMPLETADA' }
    ];

    const actualizarTextoEtapa = (p) => {
        if (!loaderStatusText) return;
        for (let i = estadosTexto.length - 1; i >= 0; i--) {
            if (p >= estadosTexto[i].min) {
                loaderStatusText.textContent = estadosTexto[i].texto;
                break;
            }
        }
    };
